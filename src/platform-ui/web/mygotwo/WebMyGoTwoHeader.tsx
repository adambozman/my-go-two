...
export default function WebMyGoTwoHeader() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("web-mygotwo-notif-count")
  ...
  return (
    <header className="shrink-0 px-6 pt-[var(--header-top-padding)]">
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-4" style={{ height: "var(--header-icons-row-height)" }}>
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
        ...
        <nav className="flex min-w-0 items-start justify-center gap-3">
          {navItems.map((item) => (
            <NavLink
              key={item.url}
              to={item.url}
            ...
            >
              <span
                className="relative flex items-center justify-center rounded-full card-design-neumorph"
                style={{
              ...
              >
                ...
                {item.url === "/dashboard/notifications" && unreadCount > 0 ? (
                  <span
                    className="absolute -right-1 -top-1 flex h-5 min-w-[20px] items-center justify-center rounded-full px-0.5 text-[10px] font-bold"
      ...
      <div className="mt-[var(--header-divider-margin-top)] border-b border-border/30" />
    </header>
  );
}
