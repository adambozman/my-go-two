...
export default function InlinePhotoSearch({ imageKey, label, onImageChanged }: InlinePhotoSearchProps) {
  ...
  return <AdminPhotoSearch imageKey={imageKey} label={label} onImageChanged={onImageChanged} />;
}

function AdminPhotoSearch({ imageKey, label, onImageChanged }: InlinePhotoSearchProps) {
  ...
  const pick = useCallback(async (photo: Photo) => {
    ...
    try {
      ...
      const proxyUrl = photo.full;
      
      // Fetch the image, resize via canvas, upload
      ...
      await setImageUrl(imageKey, finalUrl);
