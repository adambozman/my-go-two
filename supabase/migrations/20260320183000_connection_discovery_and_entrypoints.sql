CREATE TABLE public.user_discovery_settings (
  ...
  share_avatar_in_discovery boolean NOT NULL DEFAULT false,
...
CREATE TABLE public.user_discovery_contacts (
  ...
  phone_search_normalized text GENERATED ALWAYS AS (
    NULLIF(regexp_replace(COALESCE(phone_raw, ''), '\D', '', 'g'), '')
  ) STORED,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TRIGGER update_user_discovery_contacts_updated_at
  BEFORE UPDATE ON public.user_discovery_contacts
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.connection_share_tokens;
