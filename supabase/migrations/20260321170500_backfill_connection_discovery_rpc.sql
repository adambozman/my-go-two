-- Backfill missing connection discovery RPC objects in environments where
-- 20260320183000 / 20260321113000 were not applied.

CREATE TABLE IF NOT EXISTS public.connection_share_tokens (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  owner_user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  token text NOT NULL UNIQUE DEFAULT encode(gen_random_bytes(24), 'hex'),
  channel text NOT NULL DEFAULT 'qr' CHECK (channel IN ('qr', 'link')),
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'update_connection_share_tokens_updated_at'
      AND tgrelid = 'public.connection_share_tokens'::regclass
  ) THEN
...
BEGIN
  ...
  INSERT INTO public.connection_share_tokens (
    owner_user_id,
    channel,
    expires_at
  )
...
BEGIN
  ...
  WHERE (
      ...
      OR
      (c.inviter_id = viewer_user_id AND c.invitee_id = token_row.owner_user_id)
    )
  ORDER BY c.created_at DESC
  LIMIT 1;

  IF existing_couple.id IS NOT NULL THEN
    UPDATE public.connection_share_tokens
    ...
    SELECT
      ...
      CASE
        ...
        ELSE 'pending_exists'
      END,
      existing_couple.inviter_id,
      existing_couple.invitee_id;
    RETURN;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.search_discoverable_users(text, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.create_connection_request(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.issue_connection_share_token(text, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.create_connection_invite_from_token(text) TO authenticated, service_role;
