...
ALTER PUBLICATION supabase_realtime ADD TABLE public.shared_derived_features;

CREATE OR REPLACE FUNCTION public.connection_can_view_card_entry(
...
AS $$
  SELECT EXISTS (
    ...
    WHERE sce.card_entry_id = p_card_entry_id
      AND sce.connection_user_id = p_viewer_id
      AND c.status = 'accepted'
      AND ce.user_id = sce.owner_user_id
  );
$$;
